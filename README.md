...
|변경 전. 이중 for문|변경 후. 단일 for문|
|:---:|:---:|
|데이터셋의 각 컬럼(N개)과 필요한 기능(M개)을 확인하는 이중 for문 사용. 이 과정에서 각 값에 대해 Aggregation, Random, TopBottom에 해당하는지 일일이 확인|사전에 Aggregation, Rangom, TopBottom가 필요한 컬럼 위치 사전에 식별하여 저장함으로써, 해당 컬럼 위치만 확인하는 단일 for문으로 변경 |
